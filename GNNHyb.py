...
from k_gnn import DataLoader, max_pool
...
parser.add_argument('-learnRate', type=float, default=0.0001)   # Learning Rate
parser.add_argument('-model', type=str, default="GATv2Conv")   # GATv2Conv or GraphConv
...
class MyPreTransform(object):
    def __call__(self, data):
        # print("49 data", data.x)
        # data.x = F.one_hot(data.x[:, 0], num_classes=2).to(torch.float)  # Convert node labels to one-hot
        # data.x = torch.tensor([ x for [x] in data.x])
        data.x = F.one_hot(data.x[:, 0], num_classes=2).to(torch.float)
        return data
...
MODEL = "GNNHyb-"+str(args.activation)+"-"+str(RANDOM_RATIO).replace(".", ",")+"-"+str(DISTRIBUTION)+"-"+str(NORM)+"-"+MODEL_NAME+"-"+args.activation+"-"+str(EPOCHS)+"-"+str(SPLITS)+"-"
...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        if deterministic_dims > 0:  # Transform the deterministic dimensions using additional GraphConv
            ...
            elif MODEL_NAME == "GATv2Conv":
                self.conv1 = GATv2Conv(dataset.num_features, 32, norm=NORM)
                self.conv2 = GATv2Conv(32, deterministic_dims, norm=NORM)
            else:
                print("No such model!")
        ...
        for layer in range(LAYERS):
            if MODEL_NAME == "GraphConv":
                ...
                self.conv_layers.append(GATv2Conv(WIDTH, WIDTH, norm=NORM))
            else:
                print("No such model!")
...
def train(epoch, loader, optimizer):
    ...
    # prev_x = torch.tensor([[0]])
    for data in loader:
    #     if len(prev_x) == len(data.x):
    #         print("same as pre?", torch.all(prev_x.eq(data['x'])))
    #     prev_x = data['x']
    #     # print("train data: ", data['x'])
...
for i in range(SPLITS):
    ...
    # print("About test_mask: ", type(test_mask), test_mask)
    ...
    print("len trian set, trainset: ", len(train_dataset), train_dataset[:5])
